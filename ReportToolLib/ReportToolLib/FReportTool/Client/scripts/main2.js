...
$(document).ready(function(){
    ...
    $(document).on('click', '.left-m-list li', function (e) {
        ...
        $('.left-m-connection').removeClass('selected');
        $('.left-m-list').hide();
        $('.refresh-reports-btn').fadeOut(300);
    });
    ...
    $(document).on('click', '.left-m-connection-name, .left-m-connection > i', function () {
        var $this = $(this);
        ...
        if ($list.css('display') == 'none') {
            ...
            $this.parents('.left-m-connection:first').addClass('selected');
            $list.slideDown(200);
        }
    ...
    $(document).on('click', '.back-btn', function () {
        $('.left-m').fadeIn(500);
        $('.refresh-reports-btn').fadeIn(500);
        $('.back-btn').fadeOut(500);
        $('.report-body-w').slideUp(1000);
    });
});
...
function refreshPreview() {
    ...
    Network.ajaxRequest({
        ...
        success: function (data) {
            ...
            setTimeout(function () {
                $('#report-w').fadeIn(1000);
                $('.report-body-w').fadeIn(1000);
            }, 500);
        }
    });
}
...
function refreshReports(showGlobalLoading, callback) {
    $('.left-m-list-w').html('<div class="left-m-list-w-c"></div>');
    $('.report-body-w').hide();
    $('.report-nf').hide();
            ...
            if (data.ResultCode == 200 && data.Reports.length > 0) {
                $(data.Reports).each(function (i, report) {
                    if ($('div.left-m-connection[data-name="' + report.ConnectionName + '"]').length == 0) {
                        $('.left-m-list-w-c').append('<div class="left-m-connection" data-name="' + report.ConnectionName + '">\
                ...
                if ($('div.left-m-connection').length < 3)
                    $('.left-m-list-w-c').css('max-width', $('div.left-m-connection').length * 300);
            }
            else {
                $('.report-body-w').hide();
                $('.report-nf').show();
